...
namespace ThoughtWorks.QRCode.Codec
{
    ...
    {
        internal QRCodeSymbol _qrCodeSymbol;
        internal int _numTryDecode;
        internal System.Collections.ArrayList _results;
        internal System.Collections.ArrayList _lastResults = System.Collections.ArrayList.Synchronized(new System.Collections.ArrayList(10));
        internal static DebugCanvas _canvas;
        internal QRCodeImageReader _imageReader;
        internal int _numLastCorrections;
        internal bool _correctionSucceeded;

        public static DebugCanvas Canvas
        {
            get
            {
                return QRCodeDecoder._canvas;
            }

            set
            {
                QRCodeDecoder._canvas = value;
            }
        ...
        {
            _numTryDecode = 0;
            _results = System.Collections.ArrayList.Synchronized(new System.Collections.ArrayList(10));
            QRCodeDecoder._canvas = new DebugCanvasAdapter();
        }
        ...
        public virtual sbyte[] decodeBytes(QRCodeImage qrCodeImage)
        {
            ...
            while (_numTryDecode < adjusts.Length)
            {
                try
                {
                    DecodeResult result = decode(qrCodeImage, adjusts[_numTryDecode]);
                    ...
                    {
                        results.Add(result);
                        _canvas.println("Decoding succeeded but could not correct");
                        _canvas.println("all errors. Retrying..");
                    }
                }
            ...
            _canvas.println("All trials need for correct error");
            _canvas.println("Reporting #" + (lowestErrorIndex) + " that,");
            _canvas.println("corrected minimum errors (" + lowestError + ")");

            _canvas.println("Decoding finished.");
        ...
        internal virtual DecodeResult decode(QRCodeImage qrCodeImage, Point adjust)
        {
            try
            {
                if (_numTryDecode == 0)
                {
                    _canvas.println("Decoding started");
                    int[][] intImage = imageToIntArray(qrCodeImage);
                    _imageReader = new QRCodeImageReader();
                    _qrCodeSymbol = _imageReader.getQRCodeSymbol(intImage);
                }
                else
                {
                    _canvas.println("--");
                    _canvas.println("Decoding restarted #" + (_numTryDecode));
                    _qrCodeSymbol = _imageReader.getQRCodeSymbolWithAdjustedGrid(adjust);
                }
            }
            catch (SymbolNotFoundException e)
            {
            ...
            _canvas.println("Created QRCode symbol.");
            _canvas.println("Reading symbol.");
            _canvas.println("Version: " + _qrCodeSymbol.VersionReference);
            _canvas.println("Mask pattern: " + _qrCodeSymbol.MaskPatternRefererAsString);
            // blocks contains all (data and RS) blocks in QR Code symbol
            int[] blocks = _qrCodeSymbol.Blocks;
            _canvas.println("Correcting data errors.");
            // now blocks turn to data blocks (corrected and extracted from original blocks)
            blocks = correctDataBlocks(blocks);
            try
            {
                sbyte[] decodedByteArray = getDecodedByteArray(blocks, _qrCodeSymbol.Version, _qrCodeSymbol.NumErrorCollectionCode);
                return new DecodeResult(this, decodedByteArray, _numLastCorrections, _correctionSucceeded);
            }
            catch (InvalidDataBlockException e)
            {
                _canvas.println(e.Message);
                throw new DecodingFailedException(e.Message);
            }
        }
        ...
        internal virtual int[] correctDataBlocks(int[] blocks)
        {
            ...
            int dataCapacity = _qrCodeSymbol.DataCapacity;
            int[] dataBlocks = new int[dataCapacity];
            int numErrorCollectionCode = _qrCodeSymbol.NumErrorCollectionCode;
            int numRSBlocks = _qrCodeSymbol.NumRSBlocks;
            ...
            if (numRSBlocks == 1)
            {
                ...
                if (numCorrections > 0)
                    _canvas.println(System.Convert.ToString(numCorrections) + " data errors corrected.");
                else
                    _canvas.println("No errors found.");
                _numLastCorrections = numCorrections;
                _correctionSucceeded = corrector.CorrectionSucceeded;
                return blocks;
            }
            else
            {
