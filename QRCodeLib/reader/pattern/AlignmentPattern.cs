...
namespace ThoughtWorks.QRCode.Codec.Reader.Pattern
{
    ...
    public class AlignmentPattern
    {
        ...
        internal int _patternDistance;

        virtual public int LogicalDistance
        {
            get
            {
                return _patternDistance;
            }
        ...
        {
            this._center = center;
            this._patternDistance = patternDistance;
        }
        ...
        internal static Point[][] getCenter(bool[][] image, FinderPattern finderPattern, Point[][] logicalCenters)
        {
            ...
            _canvas.drawCross(centers[0][0], ThoughtWorks.QRCode.Codec.Util.Color_Fields.BLUE);

            axis.Origin = finderPattern.getCenter(FinderPattern.UR);
            centers[sqrtCenters - 1][0] = axis.translate(-3, 3);
            _canvas.drawCross(centers[sqrtCenters - 1][0], ThoughtWorks.QRCode.Codec.Util.Color_Fields.BLUE);

            axis.Origin = finderPattern.getCenter(FinderPattern.DL);
            centers[0][sqrtCenters - 1] = axis.translate(3, -3);
            _canvas.drawCross(centers[0][sqrtCenters - 1], ThoughtWorks.QRCode.Codec.Util.Color_Fields.BLUE);
            ...
            for (int y = 0; y < sqrtCenters; y++)
            {
                for (int x = 0; x < sqrtCenters; x++)
                {
                    ...
                    if (y == 0)
                    {
                        ...
                        _canvas.drawCross(centers[x][y], ThoughtWorks.QRCode.Codec.Util.Color_Fields.RED);
                    }
                    else if (x == 0)
                    {
                    ...
                    if (finderPattern.Version > 1)
                    {
                        ...
                        if (centers[x][y].distanceOf(precisionCenter) < 6)
                        {
                            ...
                            _canvas.println("Adjust AP(" + x + "," + y + ") to d(" + dx + "," + dy + ")");

                            centers[x][y] = precisionCenter;
                        }
                    }
                    _canvas.drawCross(centers[x][y], ThoughtWorks.QRCode.Codec.Util.Color_Fields.BLUE);
                    _canvas.drawLine(new Line(tmpPoint, centers[x][y]), ThoughtWorks.QRCode.Codec.Util.Color_Fields.LIGHTBLUE);
