...
namespace ThoughtWorks.QRCode.Codec.Reader
{
    ...
    {
        internal int[] _blocks;
        internal int _dataLengthMode;
        internal int _blockPointer;
        internal int _bitPointer;
        internal int _dataLength;
        internal int _numErrorCorrectionCode;
        internal DebugCanvas _canvas;

        const int MODE_NUMBER = 1;
        const int MODE_ROMAN_AND_NUMBER = 2;
        const int MODE_8BIT_BYTE = 4;
        const int MODE_KANJI = 8;
        int[][] _sizeOfDataLengthInfo = new int[][] { new int[] { 10, 9, 8, 8 }, new int[] { 12, 11, 16, 10 }, new int[] { 14, 13, 16, 12 } };
        ...
        virtual internal int NextMode
        {
            get
            {
                ...
                if ((_blockPointer > _blocks.Length - _numErrorCorrectionCode - 2))
                    return 0;
                else
        ...
        virtual public sbyte[] DataByte
        {
            get
            {
                _canvas.println("Reading data blocks.");
                ...
                try
                {
                    do
                    {
                        ...
                        if (mode != MODE_NUMBER && mode != MODE_ROMAN_AND_NUMBER && mode != MODE_8BIT_BYTE && mode != MODE_KANJI)
                        {
                            ...
                            throw new InvalidDataBlockException("Invalid mode: " + mode + " in (block:" + _blockPointer + " bit:" + _bitPointer + ")");
                        }
                        _dataLength = getDataLength(mode);
                        if (_dataLength < 1)
                            throw new InvalidDataBlockException("Invalid data length: " + _dataLength);
                        ...
                        switch (mode)
                        {
                            ...
                            case MODE_NUMBER:
                                ...
                                temp_sbyteArray = SystemUtils.ToSByteArray(SystemUtils.ToByteArray(getFigureString(_dataLength)));
                            ...
                            case MODE_ROMAN_AND_NUMBER:
                                ...
                                temp_sbyteArray2 = SystemUtils.ToSByteArray(SystemUtils.ToByteArray(getRomanAndFigureString(_dataLength)));
                            ...
                            case MODE_8BIT_BYTE:
                                ...
                                temp_sbyteArray3 = get8bitByteArray(_dataLength);
                            ...
                            case MODE_KANJI:
                                ...
                                temp_sbyteArray4 = SystemUtils.ToSByteArray(SystemUtils.ToByteArray(getKanjiString(_dataLength)));
                ...
                catch (System.IndexOutOfRangeException e)
                {
                    ...
                    throw new InvalidDataBlockException("Data Block Error in (block:" + _blockPointer + " bit:" + _bitPointer + ")");
                }
        ...
        virtual public string DataString
        {
            get
            {
                ...
                do
                {
                    ...
                    _dataLength = getDataLength(mode);
                    _canvas.println(System.Convert.ToString(_blocks[_blockPointer]));
                    System.Console.Out.WriteLine("length: " + _dataLength);
                    switch (mode)
                    {

                        case MODE_NUMBER:
                            //canvas.println("Mode: Figure");
                            dataString += getFigureString(_dataLength);
                            break;

                        case MODE_ROMAN_AND_NUMBER:
                            //canvas.println("Mode: Roman&Figure");
                            dataString += getRomanAndFigureString(_dataLength);
                            break;

                        case MODE_8BIT_BYTE:
                            //canvas.println("Mode: 8bit Byte");
                            dataString += get8bitByteString(_dataLength);
                            break;

                        case MODE_KANJI:
                            //canvas.println("Mode: Kanji");
                            dataString += getKanjiString(_dataLength);
                            break;
                    }
                    //canvas.println("DataLength: " + dataLength);
                    //Console.out.println(dataString);
        ...
        public QRCodeDataBlockReader(int[] blocks, int version, int numErrorCorrectionCode)
        {
            ...
            _dataLength = 0;
            this._blocks = blocks;
            this._numErrorCorrectionCode = numErrorCorrectionCode;
            if (version <= 9)
                _dataLengthMode = 0;
                ...
                _dataLengthMode = 2;
            _canvas = QRCodeDecoder.Canvas;
        }

        internal virtual int getNextBits(int numBits)
        {
            int bits = 0;
            if (numBits < _bitPointer + 1)
            {
                ...
                mask <<= (_bitPointer - numBits + 1);

                bits = (_blocks[_blockPointer] & mask) >> (_bitPointer - numBits + 1);
                _bitPointer -= numBits;
                return bits;
            }
            else if (numBits < _bitPointer + 1 + 8)
            {
                // next word crosses 2 data blocks
                int mask1 = 0;
                for (int i = 0; i < _bitPointer + 1; i++)
                {
                    mask1 += (1 << i);
                }
                bits = (_blocks[_blockPointer] & mask1) << (numBits - (_bitPointer + 1));
                _blockPointer++;
                bits += ((_blocks[_blockPointer]) >> (8 - (numBits - (_bitPointer + 1))));

                _bitPointer = _bitPointer - numBits % 8;
                if (_bitPointer < 0)
                {
                    _bitPointer = 8 + _bitPointer;
                }
                return bits;
            }
            else if (numBits < _bitPointer + 1 + 16)
            {
                ...
                int bitsFirstBlock = (_blocks[_blockPointer] & mask1) << (numBits - (_bitPointer + 1));
                _blockPointer++;

                int bitsSecondBlock = _blocks[_blockPointer] << (numBits - (_bitPointer + 1 + 8));
                _blockPointer++;

                for (int i = 0; i < numBits - (_bitPointer + 1 + 8); i++)
                {
                    mask3 += (1 << i);
                }
                mask3 <<= 8 - (numBits - (_bitPointer + 1 + 8));
                int bitsThirdBlock = (_blocks[_blockPointer] & mask3) >> (8 - (numBits - (_bitPointer + 1 + 8)));

                bits = bitsFirstBlock + bitsSecondBlock + bitsThirdBlock;
                _bitPointer = _bitPointer - (numBits - 8) % 8;
                if (_bitPointer < 0)
                {
                    _bitPointer = 8 + _bitPointer;
                }
        ...
        internal virtual int getDataLength(int modeIndicator)
        {
            ...
            return getNextBits(_sizeOfDataLengthInfo[_dataLengthMode][index]);
        }
