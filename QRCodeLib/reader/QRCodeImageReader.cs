...
namespace ThoughtWorks.QRCode.Codec.Reader
{
    ...
    public class QRCodeImageReader
    {
        ...
        internal SamplingGrid _samplingGrid;
        internal bool[][] _bitmap;

        public QRCodeImageReader()
        {
            this._canvas = QRCodeDecoder.Canvas;
        }
        ...
        public virtual QRCodeSymbol getQRCodeSymbol(int[][] image)
        {
            ...
            _bitmap = filterImage(image);
            _canvas.println("Drawing matrix.");
            _canvas.drawMatrix(_bitmap);

            _canvas.println("Scanning Finder Pattern.");
            ...
            {
                _canvas.println("Not found, now retrying...");
                _bitmap = applyCrossMaskingMedianFilter(_bitmap, 5);
                _canvas.drawMatrix(_bitmap);
            ...
            _canvas.println("FinderPattern at");
            ...
            _canvas.println(finderPatternCoordinates);
            int[] sincos = finderPattern.getAngle();
            _canvas.println("Angle*4098: Sin " + System.Convert.ToString(sincos[0]) + "  " + "Cos " + System.Convert.ToString(sincos[1]));

            int version = finderPattern.Version;
            _canvas.println("Version: " + System.Convert.ToString(version));
            ...
            try
            {
                alignmentPattern = AlignmentPattern.findAlignmentPattern(_bitmap, finderPattern);
            }
            ...
            _canvas.println("AlignmentPatterns at");
            for (int y = 0; y < matrixLength; y++)
            {
                ...
                _canvas.println(alignmentPatternCoordinates);
            }
            //for(int i = 0; i < 500000; i++) Console.out.println("");

            _canvas.println("Creating sampling grid.");
            //[TODO] need all-purpose method
            //samplingGrid = getSamplingGrid2_6(finderPattern, alignmentPattern);
            _samplingGrid = getSamplingGrid(finderPattern, alignmentPattern);
            _canvas.println("Reading grid.");
            bool[][] qRCodeMatrix = null;
            try
            {
                qRCodeMatrix = getQRCodeMatrix(_bitmap, _samplingGrid);
            }
        ...
        {
            if (_bitmap == null || _samplingGrid == null)
            {
            ...
            _canvas.println("Sampling grid adjusted d(" + adjust.X + "," + adjust.Y + ")");
        ...
        internal virtual bool[][] getQRCodeMatrix(bool[][] image, SamplingGrid gridLines)
        {
            ...
            _canvas.println("gridSize=" + gridSize);
            ...
            _canvas.drawPoint(bottomRightPoint, ThoughtWorks.QRCode.Codec.Util.Color_Fields.BLUE);
